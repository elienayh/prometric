...
export async function fetchClassCohortStats(classId: string): Promise<StatsPayload | null> {
  ...
  for (const [sid, sEvals] of evalsByStudent.entries()) {
    ...
    students_latest.push({
      ...
      age_years: latest.age_years != null ? Number(latest.age_years) : null,
      evaluated_at: latest.evaluated_at,
      classifications: (latest.classifications ?? null) as Classifications | null,
      weight_kg: latest.weight_kg != null ? Number(latest.weight_kg) : null,
      height_cm: latest.height_cm != null ? Number(latest.height_cm) : null,
      imc: latest.imc != null ? Number(latest.imc) : null,
      sit_and_reach_cm: latest.sit_and_reach_cm != null ? Number(latest.sit_and_reach_cm) : null,
      abdominal_reps: latest.abdominal_reps != null ? Number(latest.abdominal_reps) : null,
      horizontal_jump_cm: latest.horizontal_jump_cm != null ? Number(latest.horizontal_jump_cm) : null,
      medicine_ball_m: latest.medicine_ball_m != null ? Number(latest.medicine_ball_m) : null,
      square_test_s: latest.square_test_s != null ? Number(latest.square_test_s) : null,
      sprint_20m_s: latest.sprint_20m_s != null ? Number(latest.sprint_20m_s) : null,
      run_6min_m: latest.run_6min_m != null ? Number(latest.run_6min_m) : null,
      class_id: classId,
