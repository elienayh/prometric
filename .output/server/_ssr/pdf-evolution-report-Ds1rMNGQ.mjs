...
import { a as prometricIndex, i as dimensionScores, n as PM_DIMENSIONS } from "./prometric-method-BGZfQ42Z.mjs";
import { c as situationSentence, n as EXPECTED_DIMENSION_RANGE, o as REFERENCE_LABEL, r as EXPECTED_INDEX_RANGE, s as scoreToSituation } from "./prometric-reference-G7AL_tS3.mjs";
import { t as E } from "../_libs/jspdf.mjs";
import { t as require_lib } from "../_libs/qrcode.mjs";
import { r as withConsolidatedView, t as consolidatedClassifications } from "./student-metrics-BReO5ZgL.mjs";
import { a as resolveLogoUrl, i as resolveBrandingChain, n as fetchImageDataUrl, r as hexToRgb } from "./branding-B47KYsR4.mjs";
//#region node_modules/.nitro/vite/services/ssr/assets/pdf-evolution-report-Ds1rMNGQ.js
var import_lib = /* @__PURE__ */ __toESM(require_lib());
function familyActions(s, weakest, best, delta) {
	const tips = [];
	...
	} else tips.push("Acompanhe esta evolução com regularidade no Portal do Aluno.");
...
function studentActions(s, weakest, score) {
	const tips = [];
	if (s === "Muito abaixo" || s === "Abaixo") {
...
function aggregateZones(rows) {
	const buckets = {};
	for (const r of rows) for (const [k, z] of Object.entries(r.classifications ?? {})) {
	...
	for (const [k, arr] of Object.entries(buckets)) {
		...
		out[k] = ZONES[Math.round(arr.reduce((a, b) => a + b, 0) / arr.length)];
	}
	return out;
}
async function downloadStudentEvolutionPDF(studentId, tenantId, fallbackTenantName = "ProMetric") {
	const [{ data: student }, { data: evals }, { data: tenantRow }] = await Promise.all([
		supabase.from("students").select("id, full_name, sex, birth_date, class_id, portal_enabled, portal_token, portal_slug, class:classes(name, school_id), group:groups(name, display_name, primary_color, secondary_color, description, logo_url)").eq("id", studentId).maybeSingle(),
		supabase.from("evaluations").select("id, evaluated_at, classifications").eq("student_id", studentId).order("evaluated_at", { ascending: true }),
		supabase.from("tenants").select("name, display_name, primary_color, secondary_color, description, website, email, phone, logo_url").eq("id", tenantId).maybeSingle()
	]);
	if (!student) throw new Error("Aluno não encontrado");
	const s = student;
	const history = withConsolidatedView(evals ?? []).filter((e) => Object.values(e.classifications ?? {}).some(Boolean));
	...
	const { data: schoolRow } = schoolId ? await supabase.from("schools").select("name, display_name, primary_color, secondary_color, description, logo_url").eq("id", schoolId).maybeSingle() : { data: null };
	const fetchPeerLatest = async (mode) => {
		let q = supabase.from("students").select("id, evaluations(classifications, evaluated_at)").eq("tenant_id", tenantId).eq("is_active", true);
		if (mode === "class" && classId) q = q.eq("class_id", classId);
		const { data } = await q;
		const rows = [];
		for (const ps of data ?? []) {
			const evs = ps.evaluations ?? [];
			if (!evs.length) continue;
			rows.push({ classifications: consolidatedClassifications(evs) });
		}
		return rows;
	};
	const [classPeers, schoolPeers] = await Promise.all([classId ? fetchPeerLatest("class") : Promise.resolve([]), fetchPeerLatest("school")]);
	const brand = resolveBrandingChain(s.group, schoolRow, tenantRow);
	...
	const portalUrl = !!s.portal_enabled && !!portalKey && typeof window !== "undefined" ? `${window.location.origin}${s.portal_slug ? "/p/" : "/portal/aluno/"}${portalKey}` : null;
	const qrDataUrl = portalUrl ? await import_lib.toDataURL(portalUrl, {
		margin: 0,
		width: 200
	}).catch(() => null) : null;
	...
	[...dimDeltas].sort((a, b) => a.delta - b.delta)[0];
	[...dimDeltas].sort((a, b) => Math.abs(a.delta) - Math.abs(b.delta))[0];
	...
	if (lowestNow && lowestNow.score < 50) recommendations.push(`Priorizar trabalho em ${lowestNow.dimension.toLowerCase()} (atual: ${lowestNow.score}/100).`);
	if (bestEvo && bestEvo.delta > 5) recommendations.push(`Manter a estratégia que gerou +${bestEvo.delta} pontos em ${bestEvo.dimension.toLowerCase()}.`);
	if (highestNow && highestNow.score >= 75) recommendations.push(`Estimular avanço em ${highestNow.dimension.toLowerCase()} — perfil de potencial elevado.`);
	if (recommendations.length === 0) recommendations.push("Continuar a periodização atual e reavaliar em 8–12 semanas.");
	const doc = new E({
		unit: "mm",
		format: "a4"
	});
	...
	doc.setGState(new doc.GState({ opacity: .35 }));
	doc.rect(W * .55, 0, W * .45, 20, "F");
	doc.setGState(new doc.GState({ opacity: 1 }));
	if (logoDataUrl) try {
		doc.addImage(logoDataUrl, "PNG", M, 3, 14, 14, void 0, "FAST");
	} catch {}
	doc.setTextColor(255);
	doc.setFont("helvetica", "bold").setFontSize(12);
	doc.text("RELATÓRIO EVOLUTIVO PROMETRIC®", logoDataUrl ? 28 : M, 9);
	doc.setFont("helvetica", "normal").setFontSize(8);
	doc.text(tenantName, logoDataUrl ? 28 : M, 14.5);
	doc.setFontSize(7.5);
	doc.text(`Emitido em ${(/* @__PURE__ */ new Date()).toLocaleDateString("pt-BR")}  •  ${history.length} avaliação(ões)`, W - M, 9, { align: "right" });
	...
	doc.circle(discCx, discCy, 18.2, "S");
	...
	for (let i = 0; i < arcSteps; i++) {
		...
		doc.line(discCx + Math.cos(a1) * 18.2, discCy + Math.sin(a1) * 18.2, discCx + Math.cos(a2) * 18.2, discCy + Math.sin(a2) * 18.2);
	}
	const markerA = -Math.PI / 2 + idxLast.score / 100 * 2 * Math.PI;
	doc.setFillColor(...sitTone);
	doc.circle(discCx + Math.cos(markerA) * 18.2, discCy + Math.sin(markerA) * 18.2, 1.6, "F");
	doc.setFillColor(...primary);
	doc.circle(discCx, discCy, discR, "F");
	doc.setTextColor(255);
	doc.setFont("helvetica", "bold").setFontSize(22);
	doc.text(String(idxLast.score), discCx, discCy + 1, {
		align: "center",
		baseline: "middle"
	});
	...
	doc.text(situation ?? "Sem dados", 84, y + 29, { align: "center" });
	doc.setFont("helvetica", "normal").setFontSize(6.5);
	doc.setTextColor(110);
	doc.text(`Faixa esperada para a idade: ${EXPECTED_INDEX_RANGE.min}–${EXPECTED_INDEX_RANGE.max} pts`, midX, y + 37);
	const rightHeroX = 122;
	doc.setFont("helvetica", "bold").setFontSize(13);
	doc.setTextColor(20);
	...
	[
		...
		schoolRow?.name ? `Escola: ${schoolRow.name}` : null,
		s.group?.name ? `Grupo: ${s.group.name}` : null
	].filter(Boolean).forEach((t, i) => doc.text(t, rightHeroX, y + 13 + i * 4));
	const chipY = y + heroH - 11;
	const chipColor = delta >= 0 ? [
		34,
		197,
		94
	] : [
	...
	doc.text(`${arrowEv} ${delta >= 0 ? "+" : ""}${delta} pts evolução`, 145, chipY + 5.5, { align: "center" });
	y += 45;
	...
	dimsLast.forEach((d) => {
		...
		doc.setGState(new doc.GState({ opacity: .18 }));
		doc.rect(trackX + expStart, leftY, expEnd - expStart, barH, "F");
		doc.setGState(new doc.GState({ opacity: 1 }));
		const tone = situationTone(scoreToSituation(d.score)) ?? primary;
		doc.setFillColor(...tone);
		doc.roundedRect(trackX, leftY, Math.max(.5, d.score / 100 * trackW), barH, 1, 1, "F");
	...
	[
		...
		.75,
		1
	].forEach((t) => doc.circle(cx, cy, radius * t, "S"));
	doc.setDrawColor(34, 197, 94);
	doc.setLineWidth(.4);
	...
	doc.setGState(new doc.GState({ opacity: .28 }));
	...
	if (history.length > 1) doc.text(new Date(last.evaluated_at).toLocaleDateString("pt-BR"), rightX + chartW, chartTop + chartH + 3, { align: "right" });
	...
	[{
		...
		doc.roundedRect(trackX, rightY, trackWc, 3, .5, .5, "F");
		...
		doc.setGState(new doc.GState({ opacity: 1 }));
		doc.setFillColor(...primary);
		doc.roundedRect(trackX, rightY, row.me / 100 * trackWc, 3, .5, .5, "F");
		if (row.peer !== null) {
			doc.setDrawColor(...secondary);
			doc.setLineWidth(.9);
			const px = trackX + row.peer / 100 * trackWc;
			doc.line(px, rightY - .8, px, rightY + 3.8);
		}
		doc.setFont("helvetica", "bold").setFontSize(7.5);
		doc.setTextColor(40);
	...
	doc.text("PARA A FAMÍLIA", 14, stripY + 6);
	doc.setFont("helvetica", "normal").setFontSize(7.5);
	...
	const footerY = H - 20 - 4;
	doc.setDrawColor(226, 232, 240);
	doc.setLineWidth(.3);
	doc.line(M, footerY - 2, W - M, footerY - 2);
	if (qrDataUrl) {
		try {
			doc.addImage(qrDataUrl, "PNG", M, footerY, 18, 18, void 0, "FAST");
		} catch {}
		doc.setFont("helvetica", "bold").setFontSize(8);
		doc.setTextColor(40);
		doc.text("Portal do Aluno — acompanhe em tempo real", 32, footerY + 5);
		doc.setFont("helvetica", "normal").setFontSize(7);
		doc.setTextColor(80);
		doc.text("Aponte a câmera do celular para o QR code ao lado.", 32, footerY + 9);
		if (portalUrl) {
			doc.setTextColor(...primary);
			const url = portalUrl.length > 55 ? portalUrl.slice(0, 52) + "..." : portalUrl;
			doc.text(url, 32, footerY + 13);
		}
	} else {
	...
	doc.text(`Relatório oficial ProMetric® · ${tenantName} · ${(/* @__PURE__ */ new Date()).toLocaleString("pt-BR")}`, W - M, H - 4, { align: "right" });
	const safeName = s.full_name.normalize("NFD").replace(/[\u0300-\u036f]/g, "").replace(/[^a-zA-Z0-9]+/g, "-");
	doc.save(`relatorio-evolutivo-${safeName}.pdf`);
}
//#endregion
export { downloadStudentEvolutionPDF as t };
