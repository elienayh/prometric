-- Migration: fix class_stats and group_stats to safely include all students and evaluations
-- Ensures active students (even if is_active is null or not strictly true) feed cohort statistics
...
BEGIN
  ...
  WITH origin AS (
    ...
    JOIN public.students s ON s.class_id = gs.class_id AND COALESCE(s.is_active, true)
    JOIN public.evaluations e ON e.student_id = s.id
  ...
  WITH school_latest AS (
    ...
    JOIN public.students s ON s.class_id = c2.id AND COALESCE(s.is_active, true)
    JOIN public.evaluations e ON e.student_id = s.id
